# Weekly Product Analytics Digest runbook

Status: **DOCUMENTED_NOT_DEPLOYED**
Owner: **Product Analytics owner**
Workflow: automation/n8n/workflows/weekly-product-analytics-digest.json
Trigger: Monday 10:00 Asia/Kolkata internal schedule

## Purpose and permitted actions

- query allowlisted aggregate PostHog events only
- summarize funnel and failed CTAs
- summarize setup-required friction
- prepare privacy-safe digest

Never use financial data, session replay, raw properties, distinct IDs, emails, or unapproved events.
...
- Create STOCKPRO_WF_WEEKLY_PRODUCT_ANALYTICS_DIGEST_ENABLED as false.
...
- Provision STOCKPRO_PRODUCT_ANALYTICS_ADAPTER_URL as fixed allowlisted HTTPS egress and bind the least-privilege credential named StockPro PostHog aggregate read-only adapter.
...
Disable on unexpected output, privacy signal, duplicate side effect, or adapter failure; preserve body-free evidence and escalate to Product Analytics owner and Security. Sensitive exposure also escalates to privacy owner/incident commander.
