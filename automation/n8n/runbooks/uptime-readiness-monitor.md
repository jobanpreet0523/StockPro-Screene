# Uptime and Readiness Monitor runbook

Status: **DOCUMENTED_NOT_DEPLOYED**
Owner: **Site Reliability owner**
Workflow: automation/n8n/workflows/uptime-readiness-monitor.json
Trigger: Five-minute internal schedule

## Purpose and permitted actions

- GET fixed homepage/status endpoints
- check auth, database, provider, contact, broker-vault, CRT-storage, saved-research, and waitlist readiness
- classify operational, degraded, setup_required, or outage
- notify only when the state fingerprint changes

setup_required is configuration state, not outage. Reachability is not readiness. Report current setup-required gates and contact 404 exactly; never auto-configure.
...
- Create STOCKPRO_WF_UPTIME_READINESS_MONITOR_ENABLED as false.
...
- Provision STOCKPRO_READINESS_CHECK_ADAPTER_URL as fixed allowlisted HTTPS egress and bind the least-privilege credential named StockPro readiness read-only adapter.
...
Disable on unexpected output, privacy signal, duplicate side effect, or adapter failure; preserve body-free evidence and escalate to Site Reliability owner and Security. Sensitive exposure also escalates to privacy owner/incident commander.
