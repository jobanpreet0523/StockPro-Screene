...
Owner: **Developer Experience / CI owner**
Workflow: automation/n8n/workflows/ci-failure-triage.json
Trigger: Authenticated GitHub Actions failure webhook after ingress verification and minimization

## Purpose and permitted actions

- retrieve allowlisted failed-step metadata
- classify build, test, configuration, or security failure
- create/update one deduplicated GitHub issue
- notify the operator

Never push, commit, open a code-changing PR, merge, deploy, or edit repository contents.

## Input, privacy, and prompt-injection contract

Allowed workflow-specific fields: repositoryAlias, workflowName, runReference, failedJob, failureFingerprint, logReference
...
- Create STOCKPRO_WF_CI_FAILURE_TRIAGE_ENABLED as false.
- Keep STOCKPRO_AUTOMATION_TEST_MODE=true and STOCKPRO_AUTOMATION_KILL_SWITCH=true until approval.
- Provision STOCKPRO_CI_FAILURE_TRIAGE_ADAPTER_URL as fixed allowlisted HTTPS egress and bind the least-privilege credential named StockPro CI failure triage adapter.
...
Disable on unexpected output, privacy signal, duplicate side effect, or adapter failure; preserve body-free evidence and escalate to Developer Experience / CI owner and Security. Sensitive exposure also escalates to privacy owner/incident commander.
