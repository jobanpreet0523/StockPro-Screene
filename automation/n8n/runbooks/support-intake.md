# Support Intake runbook

Status: **DOCUMENTED_NOT_DEPLOYED**
Owner: **Customer Support owner**
Workflow: automation/n8n/workflows/support-intake.json
Trigger: Authenticated support/contact-record webhook after ingress verification and minimization

## Purpose and permitted actions

- sanitize the structured projection
- classify category and assign priority
- create a support task
- send an approved acknowledgement by account reference
- record acknowledgement success only after Resend confirms delivery

Never place raw contact text in n8n, a model, an issue, notification, audit, retry, or dead letter. Prompt-injection signals require manual privacy review.

## Input, privacy, and prompt-injection contract

Allowed workflow-specific fields: supportRecordReference, structuredCategory, structuredPriority, locale, promptInjectionSignal
...
- Acknowledgement receives only opaque references and approved template version.
- Non-confirmed Resend result records no success and is audited/dead-lettered.

## Disable, escalation, and incident shutdown

Disable on unexpected output, privacy signal, duplicate side effect, or adapter failure; preserve body-free evidence and escalate to Customer Support owner and Security. Sensitive exposure also escalates to privacy owner/incident commander.
