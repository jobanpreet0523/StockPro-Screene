# Beta User Onboarding runbook

Status: **DOCUMENTED_NOT_DEPLOYED**
Owner: **Beta Program owner**
Workflow: automation/n8n/workflows/beta-user-onboarding.json
Trigger: Authenticated approved-beta-account webhook after ingress verification and minimization

## Purpose and permitted actions

- send approved welcome template
- include educational disclaimer and setup guide
- state own-broker and no-trade-execution rules
- state no paid activation

Never activate payment, imply advice, trade, request broker credentials, or include portfolio data.

## Input, privacy, and prompt-injection contract

Allowed workflow-specific fields: accountReference, locale, templateVersion
...
- Create STOCKPRO_WF_BETA_USER_ONBOARDING_ENABLED as false.
...
- Provision STOCKPRO_BETA_ONBOARDING_ADAPTER_URL as fixed allowlisted HTTPS egress and bind the least-privilege credential named StockPro approved onboarding email adapter.
...
Disable on unexpected output, privacy signal, duplicate side effect, or adapter failure; preserve body-free evidence and escalate to Beta Program owner and Security. Sensitive exposure also escalates to privacy owner/incident commander.
