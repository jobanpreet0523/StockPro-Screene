...
var NSE_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36";
var NSE_HOME = "https://www.nseindia.com";
var nseCookies = "";
var nseCookieExpiry = 0;
  ...
  if (env.STOCKPRO_KV) {
    try {
      ...
      if (cached) {
        ...
        if (Date.now() - parsed.ts < CACHE_TTL * 1e3) {
...
var YAHOO_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36";
...
var AUTH_TOKEN = "Bearer StockProSecureToken2026!";
var PUBLIC_PATHS = [
  "/api/indices",
  "/api/stocks",
  "/api/market-indices",
  ...
  try {
    ...
    if (path.startsWith("/api/yahoo-finance/")) {
      ...
      if (symbol === "quotes") {
        ...
        const res2 = await fetch(urlQuotes, { headers: { "User-Agent": YAHOO_UA, Accept: "application/json" } });
        return new Response(res2.body, { status: res2.status, headers: jsonHeaders({ "Content-Type": "application/json" }) });
      }
      ...
      if (modules) {
        ...
        const res2 = await fetch(urlSum, { headers: { "User-Agent": YAHOO_UA, Accept: "application/json" } });
        return new Response(res2.body, { status: res2.status, headers: jsonHeaders({ "Content-Type": "application/json" }) });
      }
    ...
    if (path === "/api/news" || path === "/api/market-news") {
      ...
      for (const feedUrl of feeds) {
        try {
          const res = await fetch(feedUrl, { headers: { "User-Agent": NSE_UA, Accept: "application/rss+xml,application/xml,text/xml" }, signal: AbortSignal.timeout(8e3) });
          if (!res.ok) continue;
    ...
    if (path === "/api/block-deals" || path === "/api/bulk-deals") {
      const deals = [
        { date: "2026-05-15", symbol: "RELIANCE", clientName: "Morgan Stanley Asia", buySell: "BUY", quantity: 58e4, price: 2450.5, value: 142.12 },
        { date: "2026-05-15", symbol: "TCS", clientName: "LIC of India", buySell: "BUY", quantity: 12e4, price: 3890.2, value: 46.68 },
        { date: "2026-05-14", symbol: "HDFCBANK", clientName: "Societe Generale", buySell: "SELL", quantity: 95e4, price: 1620.15, value: 153.91 },
        { date: "2026-05-14", symbol: "INFY", clientName: "Goldman Sachs", buySell: "BUY", quantity: 3e5, price: 1410.5, value: 42.31 },
        { date: "2026-05-13", symbol: "ICICIBANK", clientName: "Fidelity Investments", buySell: "SELL", quantity: 8e5, price: 1120, value: 89.6 }
      ];
    ...
    if (path === "/api/data") {
      ...
      const meta = chart?.chart?.result?.[0]?.meta;
      const spot = meta?.regularMarketPrice || 24e3;
...
function buildFallbackChain(symbol, spot) {
  ...
  for (let i = -10; i <= 10; i++) {
    ...
    const cOI = Math.round(8e4 * Math.exp(-d * 4) * (strike > spot ? 1.3 : 0.6) * (1 + Math.random() * 0.2));
    const pOI = Math.round(9e4 * Math.exp(-d * 4) * (strike < spot ? 1.4 : 0.5) * (1 + Math.random() * 0.2));
    tCallOI += cOI;
    tPutOI += pOI;
    options.push({
      strikePrice: strike,
      callLtp: +(cInt + tv + Math.random() * 2).toFixed(2),
      callChange: +((Math.random() - 0.45) * 12).toFixed(2),
      callVol: Math.round(cOI * (1.3 + Math.random())),
      callOi: cOI,
      callOiChg: Math.round((Math.random() - 0.3) * cOI * 0.12),
      callIv: +(14 + d * 55 + Math.random() * 2).toFixed(2),
      putLtp: +(pInt + tv + Math.random() * 2).toFixed(2),
      putChange: +((Math.random() - 0.55) * 12).toFixed(2),
      putVol: Math.round(pOI * (1.1 + Math.random())),
      putOi: pOI,
      putOiChg: Math.round((Math.random() - 0.4) * pOI * 0.1),
...
function parseRSS(xml, source) {
  ...
  for (let i = 1; i < items.length && articles.length < 25; i++) {
    ...
    if (title) articles.push({ title: title.replace(/&amp;/g, "&").trim(), link: link || "#", pubDate: pubDate ? new Date(pubDate).toISOString() : (/* @__PURE__ */ new Date()).toISOString(), source });
  }
  return articles;
}
...
async function getProData(symbol) {
  try {
    ...
    const marketCap = result.summaryDetail.marketCap?.raw || 1e11;
    const revenue = result.financialData.totalRevenue?.raw || 5e10;
    const netIncome = result.defaultKeyStatistics.netIncomeToCommon?.raw || 1e10;
    ...
    const relativeValue = Math.min(5, Math.max(1, Math.round(marketCap / 5e11 + 1)));
    ...
    const statementYears = statementHistory.map((item) => ({
      year: new Date(item.endDate?.raw * 1e3).getFullYear(),
...
function generateFallbackProData(symbol) {
  const price = 311.23, fairValue = 373.1;
  return {
    symbol: symbol.toUpperCase(),
