...
(function () {
  ...
  const REFRESH_MS = 4000; // refresh more frequently for responsive live feel!
  ...
  async function fetchLiveData() {
    ...
    // Now let's fetch the REAL option chain from our server if backend is ok!
    if (state.backendOk) {
      try {
        const oRes = await fetch(`${API}/api/option-chain/NIFTY`);
        if (oRes.ok) {
          ...
          if (oData.status === 'ok') {
            ...
            state.callOI = chain.totalCallOi;
            state.putOI = chain.totalPutOi;
            
            // Map options list to rows expected by the live-data.js UI
            ...
            // Patch UI and bypass simulation
            patchAll();
            return;
          }
        }
      } catch (err) {
        console.warn("Real-time option-chain API fetch failed. Reverting to simulator fallback.", err);
      }
    }
