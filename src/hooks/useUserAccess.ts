...
async function loadAccess() {
  const result = await readApi<{ status?: string; subscription?: { status?: string }; message?: string }>(
    '/api/trial/status',
    {},
    authenticatedFetch,
  );
  const payload = result.payload;
  if (result.state === 'setup_required') return { tier: 'setup_required' as const, message: payload.message || 'Access storage requires setup.' };
