...
export default function StrategyBuilder() {
  ...
  const stats = useMemo(() => {
    ...
    let displayMaxProf = maxProf > 500000 ? 'Unlimited' : `₹${typeof maxProf === 'number' ? maxProf.toFixed(2) : Number(maxProf || 0).toFixed(2)}`;
    let displayMaxLoss = maxLoss < -500000 ? 'Unlimited' : `₹${typeof maxLoss === 'number' ? maxLoss.toFixed(2) : Number(maxLoss || 0).toFixed(2)}`;
    ...
    if (typeof maxProf === 'number' && typeof maxLoss === 'number' && maxLoss < 0 && maxProf > 0) {
      const rrVal = (maxProf / Math.abs(maxLoss ?? 1));
      rr = `1 : ${typeof rrVal === 'number' ? rrVal.toFixed(2) : Number(rrVal || 0).toFixed(2)}`;
                 ...
                 <div className="grid grid-cols-3 gap-3">
                    <div className="bg-slate-800/30 p-2.5 rounded-lg border border-slate-700/30">
                      ...
                      <span className={`text-xs font-bold font-mono ${stats.netPremium >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {stats.netPremium >= 0 ? '+' : ''}{typeof stats.netPremium === 'number' ? stats.netPremium.toFixed(2) : Number(stats.netPremium || 0).toFixed(2)}
                      </span>
                    </div>
                    <div className="bg-slate-800/30 p-2.5 rounded-lg border border-slate-700/30">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData} margin={{ top: 20, right: 30, left: 10, bottom: 5 }}>
                    ...
                    <Tooltip 
                      ...
                      itemStyle={{ color: '#fff' }}
                      formatter={(value: number) => [`₹${typeof value === 'number' ? value.toFixed(2) : Number(value || 0).toFixed(2)}`, 'P&L']}
