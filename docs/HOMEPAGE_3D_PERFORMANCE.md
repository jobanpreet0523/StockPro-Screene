...
- Five platform-neutral checked-in visual baselines cover the complete desktop, tablet, mobile, reduced-motion, and WebGL-disabled homepages. Separate viewport baselines and a byte comparison prove that the WebGL render differs from the disabled fallback.
