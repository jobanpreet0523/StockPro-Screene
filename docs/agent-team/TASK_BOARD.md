...
| Backend and Supabase Engineer | Complete: protected proof | `scripts/verify-supabase-rls.mjs`; relevant Supabase docs | Protected run #237 passed real two-user/anonymous/service-only isolation, recovery-link generation, and cleanup; post-run temporary counts are zero |
| DevOps and Release Engineer | Complete: focused commit | Workflows, readiness verifier, launch checklist | Production/preview failure classification, protected Worker-secret synchronization, declared table bindings, and honest external actions |
...
| QA and Accessibility Engineer | Complete: local evidence | Required landing tests and baselines | 35/35 landing/3D, 61/61 product E2E, and 8/8 accessibility checks pass; portable static full-page baselines plus focused WebGL pixels, low-power, SPA disposal, mobile, reduced-motion, chunk failure and context failure |
